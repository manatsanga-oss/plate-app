"""
ย้าย/เพิ่ม actions ของ branch_master เข้า master-data-api
"""
...
# Find Switch node — could be one or several
sw = None
for n in wf['nodes']:
    if n.get('type')=='n8n-nodes-base.switch':
        sw = n; break
if not sw:
    print("ERROR: no Switch node found"); sys.exit(1)
sw_name = sw.get('name')
print(f"Switch node: {sw_name}")
    ...
    if not any(r.get('outputKey')==key for r in rules):
        rules.append({
            "conditions":{"options":{"caseSensitive":True,"leftValue":"","typeValidation":"strict","version":2},
                "conditions":[{"id":f"bm{offset}","leftValue":"={{ $json.body.action }}","rightValue":key,

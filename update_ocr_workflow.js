...
const wf = JSON.parse(fs.readFileSync('office_api_single.json', 'utf8'));
...
wf.nodes = wf.nodes.filter(n => !oldIds.has(n.id));
...
const newNodes = [
  ...
  },
  {
    ...
    credentials: { mistralCloudApi: { id: "o6VWlsFkBOxJYLAZ", name: "Mistral Cloud account" } },
  ...
  },
  {
    ...
    credentials: { openAiApi: { id: "odRg3D1h42QKDeOx", name: "OpenAi account" } },
...
delete conn['Code Prepare OCR'];
delete conn['HTTP Request OCR'];
...
fs.writeFileSync('office_api_single.json', JSON.stringify(wf, null, 2), 'utf8');

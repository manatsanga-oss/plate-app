...
export default function PettyCashPostagePage({ currentUser }) {
  ...
  async function saveEditBranch() {
    ...
    try {
      const res = await fetch(API_URL, { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "update_postage_doc_branch", id: editBranchDoc.id, branch_code: editBranchVal.split(" ")[0], branch_name: editBranchVal }) });
      ...
      if (data && (data.success === true || data.id || data?.[0]?.success === true || data?.[0]?.id)) {
        ...
        setMessage("❌ บันทึกไม่สำเร็จ — n8n ยังไม่มี action update_postage_doc_branch (ต้อง re-import Petty_Cash_API)");
      }
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="data-table" style={{ fontSize: 13 }}>
          <thead><tr>
            ...
            <th>#</th><th>เลขที่</th><th>วันที่</th><th>บริษัท</th><th>สาขา</th><th>ผู้เบิก</th><th>ช่วงวันที่</th><th>รายการ</th><th>ยอดรวม</th><th>สถานะ</th><th>จัดการ</th>
          </tr></thead>
          <tbody>

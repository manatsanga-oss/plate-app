...
// แสดงสถานะ — ถ้ายืนยันถึงที่หมายแล้ว → "จัดส่งสำเร็จ"
const getStatusLabel = (b) => (b.arrived_at && !isCancelled(b.status)) ? "จัดส่งสำเร็จ" : (STATUS_LABEL[b.status] || b.status);
const getStatusColor = (b) => (b.arrived_at && !isCancelled(b.status)) ? "#059669" : (STATUS_COLOR[b.status] || "#d1d5db");
const isDelivered = (b) => !!b.arrived_at && !isCancelled(b.status);
function fmtArrivedAt(iso) {
  if (!iso) return "";
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (data?.success || data?.booking_id) {
        ...
        // ส่ง Flex Message พร้อมปุ่ม "ยืนยันถึงที่หมาย" → เปิด LIFF
        fetch(API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            booking_time: form.booking_time + " น.",
            ...
            dest_lat: distanceInfo?.dest_lat || null,
            dest_lng: distanceInfo?.dest_lng || null,
            ...
            <tbody>
              {filtered.map((b) => (
                <tr key={b.booking_id}>
                  ...
                  <td>
                    {b.status ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start" }}>
                        <span style={{ background: getStatusColor(b), color: "#fff", padding: "3px 10px", borderRadius: 12, fontSize: 13, whiteSpace: "nowrap" }}>
                          {getStatusLabel(b)}
                        </span>
                        {isDelivered(b) && (
                          <span style={{ fontSize: 11, color: "#6b7280", whiteSpace: "nowrap" }}>
                            🕒 {fmtArrivedAt(b.arrived_at)}
                          </span>
                        )}
                      </div>
                    ) : "-"}
                  ...
                  <td>
                    {isBooked(b.status) && !isDelivered(b) && (
        ...
        <div className="booking-mobile" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtered.map((b) => (
            <div key={b.booking_id} style={{
              ...
              borderLeft: `4px solid ${getStatusColor(b)}`,
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  {b.status && (
                    <span style={{ background: getStatusColor(b), color: "#fff", padding: "2px 10px", borderRadius: 12, fontSize: 12 }}>
                      {getStatusLabel(b)}
                    </span>
                  )}
                  {isDelivered(b) && (

...
<div class="foot"><div class="sg">ผู้ขาย</div><div class="sg">ลูกค้า / ผู้ซื้อ</div></div>
  ...
  async function uploadDoc(file, docType, label, clearFile, setUploading) {
    ...
    try {
      ...
      setMessage("✅ อัปโหลด + แจ้ง" + label + " เข้า LINE แล้ว");
      clearFile(null);
    } catch (e) {
              ...
              <CardBody>
                ...
                {!editable && sale?.sale_no && (
                  <div style={{ marginTop: 10, padding: "10px 14px", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 10, display: "flex", gap: 10, alignItems: "center", justifyContent: "center", flexWrap: "wrap" }}>
                    ...
                    <button
                      style={{ ...btn("#0369a1"), opacity: (!cosmosFile || cosmosUploading) ? 0.6 : 1, cursor: (!cosmosFile || cosmosUploading) ? "not-allowed" : "pointer" }}
                      disabled={!cosmosFile || cosmosUploading}
                      onClick={() => uploadDoc(cosmosFile, "cosmos", "ใบสมัครฮอนด้าพลัส (COSMOS)", setCosmosFile, setCosmosUploading)}
                    >
                      {cosmosUploading ? "กำลังส่ง…" : "⬆️ อัปโหลด + แจ้ง COSMOS (LINE)"}
                    </button>
                  </div>
                )}
              </CardBody>

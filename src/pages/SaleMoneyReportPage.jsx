...
export default function SaleMoneyReportPage() {
  ...
  // pivot ช่องทางรับชำระของแต่ละใบ
  const items = useMemo(() => {
    return rows
      .filter((r) => !branch || r.branch_code === branch)
      .map((r) => {
...
<div class="sub-h">เลือกระหว่างวันที่ ${esc(thaiDate(dateFrom))} ถึง ${esc(thaiDate(dateTo))}${branch ? " · สาขา " + esc(branch) : ""} · พิมพ์เมื่อ ${esc(new Date().toLocaleString("th-TH"))}</div>
<table><thead><tr>

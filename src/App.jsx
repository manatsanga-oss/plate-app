...
import PricePromoAdvicePage from "./pages/PricePromoAdvicePage";
import PriceImpactAnalysisPage from "./pages/PriceImpactAnalysisPage";
...
import SaleWizardPage from "./pages/SaleWizardPage";
import SaleMoneyReportPage from "./pages/SaleMoneyReportPage";
import PartDepositPage from "./pages/PartDepositPage";
...
import SparePartsDailyOrdersPage from "./pages/SparePartsDailyOrdersPage";
...
import HrSalaryAdjustPage from "./pages/HrSalaryAdjustPage";
...
import UsedMotoPage from "./pages/UsedMotoPage";
...
import RedPlateRefundPage from "./pages/RedPlateRefundPage";
import RedPlateDepositPage from "./pages/RedPlateDepositPage";
import DepositIncomePage from "./pages/DepositIncomePage";
...
export default function App() {
  ...
  function canAccess(page) {
    ...
    const HR_PAGES = ["hremployees", "hrsalaryadjust", "hrholidays", "hrmonthlyextras", "hrpayroll", "hrpayrollpayment", "hrpayrollaccounts", "hrtimetracking", "hrspecialcommission", "hrnormalcommission", "hrsalesrecord", "hrytdreport"];
    ...
    if (page === "salesoverview" || page === "stockonhand" || page === "mototransfer" || page === "booking" || page === "moto" || page === "pricequote" || page === "spareorder" || page === "sparedailyorders" || page === "hondadeposit" || page === "yamahaorder" || page === "yamahadeposit" || page === "repairdeposit" || page === "outsideorder" || page === "fastmoving" || page === "pettycash" || page === "postage" || page === "pettycashgeneral" || page === "pettycashoffering" || page === "claim" || page === "vehicleregistration" || page === "searchreceiptwork" || page === "bankdeposit" || page === "chequereceive" || page === "mymotoreport" || page === "mymotoregister" || page === "expensedoccheck" || page === "fuelwithdraw" || page === "pricemarkup" || page === "payment" || page === "receiptqr" || page === "receiptissue" || page === "retailsale" || page === "bookingdeposit" || page === "redplaterefund" || page === "redplatedeposit" || page === "depositincome" || page === "salewizard" || page === "usedmoto" || page === "partgiveawayreport" || page === "receiptentry") return true;
    ...
    if (page === "deliveryfee") return false;                 // เฉพาะ admin (รายงานค่านำพาจากระบบ — ย้ายมา Report Admin 2026-08-29)
    if (page === "pricepromoadvice") return false;            // เฉพาะ admin (แนะนำราคา/ค่าส่งเสริม)
    if (page === "priceimpact") return false;                 // เฉพาะ admin (วิเคราะห์ผลราคา/โปร ต่อยอดขาย)
    ...
    if (page === "receiptqrreport") return false;             // เฉพาะ admin (รายงานลูกค้าสแกน QR กรอกผ่าน LINE)
    ...
    if (page === "driver" || page === "finance" || page === "supplier" || page === "motoprice" || page === "motomodel" || page === "motoexpense" || page === "serviceexpense" || page === "generalexpense" || page === "incomecategory" || page === "expenserecord" || page === "flowexpense" || page === "advanceexpense" || page === "position" || page === "motostock" || page === "giveawayrules") return false;
  ...
  return (
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        {activeMenu === "pricepromoadvice" && canAccess("pricepromoadvice") && <PricePromoAdvicePage currentUser={currentUser} />}
        {activeMenu === "priceimpact" && canAccess("priceimpact") && <PriceImpactAnalysisPage currentUser={currentUser} />}
        ...
        {activeMenu === "receiptqrreport" && canAccess("receiptqrreport") && <ReceiptQrCustomerReportPage currentUser={currentUser} />}
        {activeMenu === "profitloss" && canAccess("profitloss") && <ProfitLossReportPage currentUser={currentUser} />}
        ...
        {activeMenu === "usedmoto" && canAccess("usedmoto") && (
          <UsedMotoPage currentUser={currentUser} />
        )}
        {activeMenu === "salewizard" && canAccess("salewizard") && (
          <SaleWizardPage currentUser={currentUser} />
        )}
        {activeMenu === "salemoneyreport" && canAccess("salemoneyreport") && (
          <SaleMoneyReportPage currentUser={currentUser} />
        )}
        {activeMenu === "partdeposit" && canAccess("partdeposit") && (
          <PartDepositPage currentUser={currentUser} />
        )}
        ...
        {activeMenu === "chequereceive" && canAccess("chequereceive") && (
          <ChequeReceivePage currentUser={currentUser} />
        )}
        ...
        {activeMenu === "redplaterefund" && canAccess("redplaterefund") && (
          <RedPlateRefundPage currentUser={currentUser} />
        )}
        {activeMenu === "redplatedeposit" && canAccess("redplatedeposit") && (
          <RedPlateDepositPage currentUser={currentUser} />
        )}
        {activeMenu === "depositincome" && canAccess("depositincome") && (
          <DepositIncomePage currentUser={currentUser} />
        )}
        ...
        {activeMenu === "sparedailyorders" && canAccess("sparedailyorders") && (
          <SparePartsDailyOrdersPage currentUser={currentUser} />
        )}
        ...
        {activeMenu === "hrsalaryadjust" && canAccess("hrsalaryadjust") && (
          <HrSalaryAdjustPage currentUser={currentUser} />
        )}
...
function Sidebar({ activeMenu, onChange, currentUser, onLogout, canAccess }) {
  ...
  const sparePages = ["spareorder", "sparedailyorders", "hondadeposit", "yamahaorder", "yamahadeposit", "repairdeposit", "outsideorder", "depositseize", "partdeposit", "hondainventory", "yamahainventory", "fastmoving", "fastmovingstock", "partimagesearch", "productgroup", "partstatusinquiry", "partorderinquiry", "partwithdrawal", "partmodelusage"];
  ...
  return (
    <aside className="sidebar">
      ...
      <MenuGroup title="Report Admin" pages={["reportadmin","retailsalereport","taxinvoicesalesreport","creditnotereport","carpaymentreport","carpaymentreportnew","salesbypayment","otherincometaxreport","registrationsummaryreport","receipttransferreport","vehiclepurchasereport","hondasalesreport","deliveryfee","pricepromoadvice","priceimpact","stockturnover","partreceiptreport","receiptqrreport"]} activeMenu={activeMenu} onChange={onChange} canAccess={canAccess}>
        ...
        <MenuItem page="receiptqrreport" label="รายงานลูกค้าสแกน QR (LINE)" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
      </MenuGroup>
      ...
      <MenuGroup title="Sales" pages={salesPages} activeMenu={activeMenu} onChange={onChange} canAccess={canAccess}>
        ...
        <MenuSubGroup title="ขายรถ" pages={["retailsale", "bookingdeposit", "redplatedeposit", "redplaterefund", "depositincome", "salewizard", "usedmoto", "salemoneyreport"]} activeMenu={activeMenu} canAccess={canAccess}>
          ...
          <MenuItem page="redplatedeposit" label="มัดจำป้ายแดง" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
          <MenuItem page="redplaterefund" label="คืนมัดจำป้ายแดง" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
          <MenuItem page="usedmoto" label="รถมือสอง (รับซื้อ/ขาย)" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
          <MenuItem page="depositincome" label="รับฝากเงิน/รายได้อื่นๆ" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
      ...
      <MenuGroup title="Spare Parts" pages={sparePages} activeMenu={activeMenu} onChange={onChange} canAccess={canAccess}>
        <MenuSubGroup title="Order System" pages={["spareorder", "yamahaorder", "repairdeposit", "outsideorder", "depositseize", "partdeposit", "sparedailyorders"]} activeMenu={activeMenu}>
          ...
          <MenuItem page="partdeposit" label="ระบบมัดจำอะไหล่" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
      ...
      <MenuGroup title="HR" pages={["hremployees","hrsalaryadjust","hrholidays","hrmonthlyextras","hrpayroll","hrpayrollpayment","hrpayrollaccounts","hrnormalcommission","hrspecialcommission","hrsalesrecord","hrtimetracking","hrytdreport"]} activeMenu={activeMenu} onChange={onChange} canAccess={canAccess}>
        <MenuItem page="hremployees" label="ข้อมูลพนักงาน" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />
        <MenuItem page="hrsalaryadjust" label="ปรับเงินเดือน/เกรด" activeMenu={activeMenu} onChange={onChange} canAccess={canAccess} />

"""
ย้าย action 'get_receipt_transfer_summary' จาก Accounting API → Registrations API
"""
...
for n in wf['nodes']:
    if n.get('name') == 'Switch Action' and n.get('type') == 'n8n-nodes-base.switch':
        ...
        if not any(r.get('outputKey') == 'get_receipt_transfer_summary' for r in rules):
            ...
            print("OK: added Switch rule 'get_receipt_transfer_summary'")
        else:
...
if (!arr.length) return [{ json: { query: "SELECT NULL::text AS account_no, NULL::text AS bank_name, NULL::text AS paid_to_vendor, 0::numeric AS total_amount, 0::int AS receipt_count WHERE FALSE" } }];
const escList = arr.map(v => "'" + String(v).replace(/'/g, "''") + "'").join(',');
// Direct: registration_submission_items.from_bank_account_id -> bank_accounts
const query = `
SELECT ba.account_no AS account_no,
       ba.bank_name AS bank_name,
       MAX(rsi.paid_to_vendor) AS paid_to_vendor,
       SUM(rsi.bill_amount) AS total_amount,
       COUNT(DISTINCT rsi.receipt_no) AS receipt_count
FROM registration_submission_items rsi
JOIN bank_accounts ba ON ba.account_id = rsi.from_bank_account_id
WHERE rsi.receipt_no IN (${escList})
  AND rsi.paid_at IS NOT NULL
  AND rsi.from_bank_account_id IS NOT NULL
GROUP BY ba.account_no, ba.bank_name
